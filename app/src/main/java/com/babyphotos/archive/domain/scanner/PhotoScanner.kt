...
class MediaStorePhotoScanner(
    ...
    override suspend fun scanPhotosSince(timestamp: Long): List<ScannedPhoto> =
        withContext(Dispatchers.IO) {
            val photos = queryImagesSince(timestamp)
            val videos = queryVideosSince(timestamp)
            (photos + videos).sortedByDescending { it.dateAdded }
        }

    private fun queryImagesSince(timestamp: Long): List<ScannedPhoto> {
    ...
    private fun queryVideosSince(timestamp: Long): List<ScannedPhoto> {
        val projection = arrayOf(
